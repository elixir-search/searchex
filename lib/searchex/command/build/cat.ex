defmodule Searchex.Command.Build.Cat do
  ...
  def create_from_params(params) do
    %Searchex.Command.Build.Cat{params: params}
    |> gen_filescans
    |> gen_docs
    |> extract_counts
  end
  ...
  def gen_filescans(catalog) do
    scans = catalog.params.doc_dirs
            ...
            |> Enum.map(fn(filename) -> Searchex.Command.Build.Catalog.Fs.generate_filescan(catalog, filename) end)
    %Searchex.Command.Build.Cat{catalog | filescans: scans}
  end

  def gen_docs(catalog) do
    docs = Searchex.Command.Build.Catalog.Doc.generate_from_catalog(catalog)
    %Searchex.Command.Build.Cat{catalog | docs: docs}
  end
