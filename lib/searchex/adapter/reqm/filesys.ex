defmodule Searchex.Adapter.Reqm.Filesys do
  ...
  def validate_file_root_presence(frame, _opts) do
    ...
    case length(frame.params.adapter.filesys_roots) do
      0 -> halt(frame, "No filesys_roots")
      _ -> frame
    end
  end
  ...
  def validate_file_roots(frame, _opts) do
    alias Searchex.Request.Util.Helpers
    badpaths = frame.params.adapter.filesys_roots
  ...
  def expand_file_roots(frame, _opts) do
    alias Searchex.Request.Util.Helpers
    new_roots  = Helpers.expanded_filesys_roots(frame)
    new_params = put_in(frame.params, [Access.key(:adapter, nil), :filesys_roots], new_roots)
